...
class LogNoteAPIService {
  final AuthAPIService _tokenManager = AuthAPIService();
  final ioClient = IOClient(HttpClient()..findProxy = (_) => "DIRECT");

  Future<ApiResponse<List<LogNote>>> fetchData(int resId, String model) async {
    try {
      final token = await _tokenManager.getValidToken();
      final response = await ioClient.get(
          Uri.parse('${Constants.getLogNoteByModel}/$resId/$model'),
          headers: {
            'Authorization': 'Bearer $token',
            'Content-Type': 'application/json'
          });
      if (response.statusCode == 200) {
        final List<LogNote> data = (json.decode(response.body) as List)
            .map((json) => LogNote.fromJson(json))
            .toList();
  ...
  Future<ApiResponse<String>> insert(
    ...
    try {
      ...
      var request = http.MultipartRequest(
          'POST', Uri.parse(Constants.createLogNoteByModel));
