...
import '../../../../models/supporthub/outside_working_hour.dart';
import 'dart:io';
import 'package:http/io_client.dart';

class ERPTeamAPIService {
  final AuthAPIService _tokenManager = AuthAPIService();
  final ioClient = IOClient(HttpClient()..findProxy = (_) => "DIRECT");

  Future<ApiResponse<List<ERPTicket>>> fetchData() async {
    try {
      final token = await _tokenManager.getValidToken();
      final response = await ioClient.get(Uri.parse(Constants.getRequestContact),
          headers: {'Authorization': 'Bearer $token'});
      if (response.statusCode == 200) {
        final List<ERPTicket> data = (json.decode(response.body) as List)
            .map((json) => ERPTicket.fromJson(json))
            .toList();
  ...
  Future<ApiResponse<List<ERPTicket>>> fetchAllDataErpTeam(String type) async {
    try {
      final token = await _tokenManager.getValidToken();
      final response = await ioClient
          .get(Uri.parse(Constants.getAllRequestTicketErp + type), headers: {
  ...
  Future<ApiResponse<String>> insert(
    ...
    try {
      ...
      var request = http.MultipartRequest(
          'POST', Uri.parse(Constants.createRequestContact));
  ...
  Future<ApiResponse<String>> update(
    ...
    try {
      final token = await _tokenManager.getValidToken();
      var request = http.MultipartRequest(
          'PUT', Uri.parse(Constants.updateRequestContact));
      request.headers['Authorization'] = 'Bearer $token';
      request.fields
          .addAll(data.map((key, value) => MapEntry(key, value.toString())));
      if (fileImages.isNotEmpty || files.isNotEmpty) {
  ...
  Future<ApiResponse> deleteImage(int id) async {
    try {
      var response = await ioClient.delete(Uri.parse("${Constants.deleteSTI}/$id"));
      if (response.statusCode == 200) {
        return ApiResponse(
            statusCode: response.statusCode,
            message: getResponseMessage(response));
  ...
  Future<List<FileAttachment>> fetchDownloadFilePath(
    ...
    try {
      for (var item in files) {
        ...
        final response = await ioClient.get(Uri.parse(fileUrl));
        if (response.statusCode == 200) {
  ...
  //Check if outside working hour
  Future<ApiResponse<OutsideWorkingHours>> checkOutsideWorkingHour() async {
    try {
      final token = await _tokenManager.getValidToken();
      final response = await ioClient
          .get(Uri.parse(Constants.checkOutsideWorkingHour), headers: {
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json'
      });
      if (response.statusCode == 200) {
        return ApiResponse(
            data: OutsideWorkingHours.fromJson(jsonDecode(response.body)),
            statusCode: response.statusCode);
      } else {
        return ApiResponse(
            error: getResponseErrorMessage(response),
