...
class ICTTeamAPIService {
  final AuthAPIService _tokenManager = AuthAPIService();
  final ioClient = IOClient(HttpClient()..findProxy = (_) => "DIRECT");

  Future<ApiResponse<List<ICTTicket>>> fetchData() async {
    try {
      final token = await _tokenManager.getValidToken();
      final response = await ioClient
          .get(Uri.parse(Constants.getICTTicketByRequestor), headers: {
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json'
      });
      if (response.statusCode == 200) {
        final List<ICTTicket> data = (json.decode(response.body) as List)
            .map((json) => ICTTicket.fromJson(json))
            .toList();
  ...
  Future<ApiResponse<String>> insert(
    ...
    try {
      ...
      var request = http.MultipartRequest(
          'POST', Uri.parse(Constants.createRequestICTTicket));
  ...
  Future<ApiResponse<String>> update(
    ...
    try {
      final token = await _tokenManager.getValidToken();
      var request = http.MultipartRequest(
          'PUT', Uri.parse(Constants.updateRequestICTTicket));
      request.headers['Authorization'] = 'Bearer $token';
      request.fields
          .addAll(data.map((key, value) => MapEntry(key, value.toString())));
      if (fileImages.isNotEmpty || files.isNotEmpty) {
