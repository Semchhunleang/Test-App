...
class LeaveTypeAPIService {
  ...
  Future<ApiResponse<List<LeaveType>>> fetchLeaveType(
      String start, String end, bool isHalfDay) async {
    final token = await _tokenManager.getValidToken();

    try {
      var response = await ioClient.get(
        Uri.parse(
            "${Constants.getLeaveTypeByTokenAndCondition}/${start.toString()}/${end.toString()}/$isHalfDay"),
        headers: {
      ...
      if (response.statusCode == 200) {
        final List<LeaveType> responseData =
