...
import 'package:umgkh_mobile/models/supporthub/outside_working_hour.dart';
...
class RequestContactViewModel extends ChangeNotifier {
  bool loaded = false;
  ...
  OutsideWorkingHours? _outsideworkinghour;
  OutsideWorkingHours? get outsideworkinghour => _outsideworkinghour;
  ...
  Future<void> fetchData() async {
    resetFilter();
    _isLoading = true;
    final response = await ERPTeamAPIService().fetchData();
  ...
  Future<void> checkOutsideWorkingHours() async {
    final response = await ERPTeamAPIService().checkOutsideWorkingHour();
    _apiResponse = response;
    if (response.error != null) {
      _errorMessage = response.error;
      debugPrint("_errorMessage: $_errorMessage");
    } else if (response.data != null) {
      _outsideworkinghour = response.data!;
      debugPrint(
          "_outsideworkinghour: ${_outsideworkinghour!.isOutsideWorkHours}");
    }
    notifyListeners();
  }
}
