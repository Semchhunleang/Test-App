...
import 'package:umgkh_mobile/views/pages/supporthub/erp_team/erp/contact_product/form.dart';
import 'package:umgkh_mobile/widgets/button_custom.dart';

class ItemContactProductWidget extends StatelessWidget {
  final ERPTicket data;
  final String type;
  const ItemContactProductWidget(
      {super.key, required this.data, required this.type});

  @override
  Widget build(BuildContext context) => Consumer<
          RequestContactProductViewModel>(
      builder: (context, viewModel, _) => GestureDetector(
          onTap: () async {
            await navPush(
                context,
                RequestContactAndProductFormAndInfoPage(
                    data: data, type: type));
  ...
  bool hasContact() => data.contact.isNotEmpty;
