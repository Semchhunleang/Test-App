...
Widget selectActionBy(
        ...
        required dynamic selected,
        required dynamic Function(dynamic) onChanged}) =>
    Consumer<FieldServiceViewModel>(
      builder: (context, viewModel, _) => CustomDropList(
          titleHead: 'Action By',
          isValidate: validate,
          selected: selected,
          isSearch: true,
          items: viewModel.optionUser ?? [],
          itemAsString: (i) => i.name.toString(),
          onChanged: onChanged),
    );
...
imageFieldServiesLocal(BuildContext context, List<File> datas) => Wrap(
      crossAxisAlignment: WrapCrossAlignment.center,
      children: datas.asMap().entries.map((entry) {
        return widgetImgLocal(entry.value, onTap: () {
          // navPush(
          //     context,
          //     ViewFullImagePage(
          //       index: entry.key,
...
// =============================== DISPATCH FROM
Widget selectDispatchFrom(
        {required bool validate,
        required dynamic selected,
        required dynamic Function(dynamic) onChanged}) =>
    Consumer<SelectionsViewModel>(
      builder: (context, viewModel, _) => CustomDropList(
          titleHead: 'Dispatch From',
          isValidate: validate,
          selected: selected,
          items: viewModel.dispatchFrom.toList(),
          itemAsString: (i) => i.name.toString(),
          onChanged: onChanged),
    );

// =============================== ARRVICE AT
Widget selectArriveAt(
        {required bool validate,
        required dynamic selected,
        required dynamic Function(dynamic) onChanged}) =>
    Consumer<SelectionsViewModel>(
      builder: (context, viewModel, _) => CustomDropList(
          ...
          isValidate: validate,
          selected: selected,
          items: viewModel.arriveAt.toList(),
          itemAsString: (i) => i.name.toString(),
          onChanged: onChanged),
    );
...
Widget widgetImgLocal(File data, {Function()? onTap}) => Padding(
    padding: const EdgeInsets.only(right: 5),
    child: GestureDetector(
      onTap: onTap,
      child: ClipRRect(
        ...
        child: Image.file(
          ...
          //       height: imgSize,
          //       width: imgSize,
          //       child: const Center(child: WidgetLoadImage(),),);
          // },
          errorBuilder: (context, error, stackTrace) =>
              Container(height: imgSize, width: imgSize, color: Colors.white),
        ),
